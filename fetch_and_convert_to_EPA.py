...
The U.S. EPA defines AQI using pollutant-specific averaging periods and units:
  - PM2.5, PM10  → µg/m³ (24-hour average)
...
def convert_units(pollutant: str, value: float) -> float:
    ...
    For particulate matter, value is returned unchanged (already µg/m³).
    """
...
def calc_overall_aqi(row):
    ...
    # PMs use 24-hour rolling averages (µg/m³)
    if not pd.isna(row.get("pm2_5_24hr_avg")):
        aqi_vals.append(calc_aqi_for_pollutant("pm2_5", row["pm2_5_24hr_avg"]))
    if not pd.isna(row.get("pm10_24hr_avg")):
        aqi_vals.append(calc_aqi_for_pollutant("pm10", row["pm10_24hr_avg"]))

    # Gases (converted units)
...
def json_to_dataframe(json_data):
    """Convert OpenWeather JSON → DataFrame with dual unit columns and rolling averages."""
    ...
    # --- Converted columns (µg/m³ → ppb/ppm) ---
    ...
    # --- Rolling averages for regulatory compliance ---
    df["pm2_5_24hr_avg"] = df["pm2_5"].rolling(24, min_periods=18).mean()
    df["pm10_24hr_avg"] = df["pm10"].rolling(24, min_periods=18).mean()
    ...
    cols = [
        ...
        "pm2_5", "pm2_5_24hr_avg",
        "pm10", "pm10_24hr_avg",
        "co", "co_ppm", "co_ppm_8hr_avg",
        "o3", "o3_ppb", "o3_ppb_8hr_avg",
        "no2", "no2_ppb", "so2", "so2_ppb"
    ]
    return df[cols]
...
if __name__ == "__main__":
    ...
    if df is not None and not df.empty:
        ...
        df.to_csv("karachi_aqi_data_dual_units.csv", index=False)
        print(f"\n💾 Saved {len(df)} records → karachi_aqi_data_dual_units.csv")
        print("ℹ️ Note: Initial 8- or 24-hour rows show NaN due to averaging windows.")
    else:
